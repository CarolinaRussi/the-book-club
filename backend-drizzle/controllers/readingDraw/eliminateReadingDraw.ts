...
export const eliminateReadingDrawHandler = async (
  req: Request,
  res: Response,
) => {
  ...
  try {
    const readingDraw = await eliminateReadingDrawNomination(id, hostUserId);
    const message =
      readingDraw.status === "awaiting_book"
        ? "Último livro restante! Hora de cadastrar na biblioteca."
        : "Um livro foi eliminado.";
    ...
    console.error(error);
    return res.status(500).json({ message: "Erro ao eliminar indicação." });
  }
};
