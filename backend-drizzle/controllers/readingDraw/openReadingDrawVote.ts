...
export const openReadingDrawVoteHandler = async (
  req: Request,
  res: Response,
) => {
  ...
  try {
    const readingDraw = await openReadingDrawVote(id, hostUserId);
    ...
    console.error(error);
    return res.status(500).json({ message: "Erro ao abrir a votação." });
  }
};
