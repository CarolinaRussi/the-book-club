...
export const deleteMeetingRecap = async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    await meetingRecapService.deleteMeetingRecap(id);
    return res.status(200).json({
      message: "Registro do encontro removido com sucesso",
    });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "Erro ao remover registro do encontro" });
  }
};
